from build_agent import PDFProcessor
...
class PaperAgent:
    ...
    def build_knowledge_base(self, pdf_directory, index_name="default_index"):
        ...
        all_chunks, metadata = self.pdf_processor.load_pdfs_from_directory(pdf_directory)
        ...
        self.index, self.chunks, self.metadata = self.vector_builder.build_pdf_vector_store(
    ...
    def load_knowledge_base(self, index_name="default_index"):
        ...
        self.index, self.chunks, self.metadata = self.vector_builder.load_vector_store(index_name)
        return self.index is not None
    ...
    def query(self, question, k=5):
        ...
        context_metadata = [self.metadata[i] for i in indices[0] if i < len(self.metadata)]
